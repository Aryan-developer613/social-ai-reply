...
def get_prompt_template_by_id(db: Client, template_id: int) -> dict[str, Any] | None:
    ...
    return _map_prompt_template(result.data[0]) if result.data else None


def _map_prompt_template(template: dict[str, Any] | None) -> dict[str, Any] | None:
    """Normalize legacy DB prompt column names to API names."""
    if not template:
        return template
    mapped = dict(template)
    if "prompt_type" not in mapped and "type" in mapped:
        mapped["prompt_type"] = mapped["type"]
    if "instructions" not in mapped and "user_prompt_template" in mapped:
        mapped["instructions"] = mapped["user_prompt_template"] or ""
    return mapped


def _prompt_template_for_db(template_data: dict[str, Any]) -> dict[str, Any]:
    """Normalize API prompt names to the current Supabase table schema."""
    data = dict(template_data)
    if "prompt_type" in data and "type" not in data:
        data["type"] = data.pop("prompt_type")
    if "instructions" in data and "user_prompt_template" not in data:
        data["user_prompt_template"] = data.pop("instructions")
    return data
...
def create_prompt_template(db: Client, template_data: dict[str, Any]) -> dict[str, Any]:
    ...
    result = db.table(PROMPT_TEMPLATES_TABLE).insert(_prompt_template_for_db(template_data)).execute()
    return _map_prompt_template(result.data[0])  # type: ignore[return-value]


def update_prompt_template(db: Client, template_id: int, update_data: dict[str, Any]) -> dict[str, Any] | None:
    """Update a prompt template."""
    result = db.table(PROMPT_TEMPLATES_TABLE).update(_prompt_template_for_db(update_data)).eq("id", template_id).execute()
    return _map_prompt_template(result.data[0]) if result.data else None
...
def list_prompt_templates_for_project(db: Client, project_id: int) -> list[dict[str, Any]]:
    ...
    return [mapped for row in result.data if (mapped := _map_prompt_template(row))]
